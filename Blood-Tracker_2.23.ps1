...
$Version = "2.22.1" # Aktuelle Versionierung
        ...
        GeneticPredispositions = @{
            ...
            BuiltIn = @(
                @{
                    ...
                    Active  = $false
                    Markers = @("D-Dimer", "C-reaktives Protein (CRP)")
                ...
                @{
                    ...
                    Active  = $false
                    Markers = @("Homocystein", "Lp(a)", "NT-proBNP", "TnT (Troponin T)", "Systolischer Blutdruck", "ApoB", "PREVENT-ASCVD-10Y")
...
function Show-DataEntryForm {
    ...
    $markerComboBox = New-Object System.Windows.Forms.ComboBox; $markerComboBox.Location = New-Object System.Drawing.Point(110, 60); $markerComboBox.Size = New-Object System.Drawing.Size(300, 20); $entryGroup.Controls.Add($markerComboBox)
    ...
    # v2.20.0: Marker-Auswahl steuert HIV/Testo-UI
    $markerComboBox.Add_SelectedIndexChanged({
        $sel = $markerComboBox.SelectedItem
        # HIV: ComboBox statt TextBox
        $isHIV = $sel -eq "HIV (Anti-HIV-1/2)"
        $hivComboBox.Visible = $isHIV
        $valueTextBox.Visible = -not $isHIV
        # Testosteron: Einheiten-Combo
        $isTesto = $sel -eq "Testosteron, gesamt"
    ...
    function Update-AllMarkerDropdowns {
        ...
        foreach ($marker in $currentMarkers | Sort-Object Name) { $markerComboBox.Items.Add($marker.Name); $deleteMarkerComboBox.Items.Add($marker.Name) }
    ...
    function Invoke-CustomReportPrint {
        ...
        $printDoc.Add_PrintPage({
            ...
            while ($script:reportSectionIndex -lt $script:reportSections.Count) {
                ...
                if ($script:reportSectionPhase -eq 2) {
                    ...
                    while ($script:reportSectionRowIndex -lt $items.Count) {
                        ...
                        $notiz = if ($it.PSObject.Properties.Name -contains "Note") { [string]$it.Note } else { "" }
                        $vals = @([string]$dateDisp, [string]$it.Name, [string]$it.Value, [string]$it.Unit, $notiz)
                        $xPos = $margins.Left
                        for ($i = 0; $i -lt 5; $i++) {
                            $w = [int]($margins.Width * $weights[$i])
                            $val = $vals[$i]
            ...
            # Footer
            $footerText = "Seite $($script:reportPageNum) – Blood-Tracker Custom Report"
            $g.DrawString($footerText, $smallFont, [System.Drawing.Brushes]::DarkSlateGray, $margins.Left, ($margins.Top + $margins.Height - 15))
            $ev.HasMorePages = $false
        })
    ...
    $addButton.Add_Click({ 
        try { 
            ...
            $markerName = $markerComboBox.SelectedItem
            if ([string]::IsNullOrWhiteSpace($markerName)) { throw "Bitte wählen Sie einen Blutmarker aus." }
    ...
    $loadMarkerForEditButton.Add_Click({
        $markerToLoad = $script:data.Config.Markers | Where-Object { $_.Name -eq $deleteMarkerComboBox.SelectedItem } | Select-Object -First 1
        if ($markerToLoad) {
    ...
    $dataMgmtTreeView.Add_AfterSelect({
        $dataMgmtDataGridView.Rows.Clear()
        $selectedNode = $dataMgmtTreeView.SelectedNode
        if (-not $selectedNode) { $dataMgmtExportButton.Enabled = $false; $dataMgmtUploadButton.Enabled = $false; $showDocButton.Visible = $false; return }
        $selectedDate = $selectedNode.Tag
        $itemsForDate = $script:allHistoricalItems | Where-Object { $_.Date -eq $selectedDate } | Sort-Object Name
        ...
        # v2.22.0: "Dokument anzeigen" nur zeigen, wenn ein Dokument (*_BLUTTEST*) existiert
        $showDocButton.Visible = $false
        $showDocButton.Tag = $null
        try {
            $ddFile, $ddDir = Get-DailyDataFilePath -dateString $selectedDate
            if (Test-Path $ddDir) {
                $docFiles = @(Get-ChildItem -Path $ddDir -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*_BLUTTEST*" })
                if ($docFiles.Count -gt 0) {
                    $newestDoc = ($docFiles | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName
                    $showDocButton.Tag = $newestDoc
                    $showDocButton.Text = if ($docFiles.Count -gt 1) { "Dokument`nanzeigen ($($docFiles.Count))" } else { "Dokument`nanzeigen" }
                    $showDocButton.Visible = $true
                }
            }
        } catch { Write-Warning "Dokument-Erkennung: $($_.Exception.Message)" }
    })
    # v2.17.0: Dokument-Upload Handler
    $dataMgmtUploadButton.Add_Click({
        ...
        $selectedDate = $selectedNode.Tag
        $ofd = New-Object System.Windows.Forms.OpenFileDialog
        $ofd.Title = "Dokument zum Bluttest hochladen"
        ...
        if ($ofd.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            try {
                ...
                if (-not (Test-Path $dailyDataDir)) { New-Item -Path $dailyDataDir -ItemType Directory -Force | Out-Null }
                # Datei nach Schema YYYY-MM-dd_BLUTTEST.ext umbenennen
                ...
                $showDocButton.Tag = $targetPath
                $showDocButton.Text = "Dokument`nanzeigen"
                $showDocButton.Visible = $true
                [System.Windows.Forms.MessageBox]::Show("Dokument erfolgreich archiviert:`n$targetPath", "Upload erfolgreich", "OK", "Information")
            } catch {
                [System.Windows.Forms.MessageBox]::Show("Fehler beim Hochladen des Dokuments: $($_.Exception.Message)", "Fehler", "OK", "Error")
            }
        }
    })
    # v2.22.0: "Dokument anzeigen" Handler - oeffnet das archivierte Dokument
    $showDocButton.Add_Click({
        $docPath = $showDocButton.Tag
        if ($docPath -and (Test-Path $docPath)) {
            try { Start-Process -FilePath $docPath }
